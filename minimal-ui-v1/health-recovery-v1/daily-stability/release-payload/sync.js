(() => {
  ...
  const UPDATE_URL = '/manual.html?v=20260914-daily-stability-1';
  ...
  function acceptRemoteState(state, nextMeta) {
  ...
  // A successful HTTP status alone is not a state-save receipt (e.g. a
  ...
  async function readStateResponse(options) {
  ...
  async function requestState() {
    const {response, body} = await readStateResponse({ credentials: 'same-origin', cache: 'no-store' });
    if (!response.ok) {
      const error = new Error(body?.message || body?.error || `HTTP ${response.status}`);
      error.status = response.status;
      throw error;
    }
    if (!validStateReply(body, true) || body.ok === false || body.revision < meta.revision) throw unconfirmedStateReply();
    return body;
  }

  async function sendState(state, { force = false, retry = true } = {}) {
    if (!appLoaded || protective) return;
    const sentRevision = meta.revision;
    const {response, body} = await readStateResponse({
      method: 'PUT',
      credentials: 'same-origin',
      cache: 'no-store',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ state, baseRevision: sentRevision, force: false })
    });
    if (response.status === 409 && retry && body?.state) {
      if (!validStateReply(body) || body.revision <= sentRevision) throw unconfirmedStateReply();
      ...
      acceptRemoteState(merged, {...meta, baseState: normalizeState(body.state), revision: body.revision});
      applyRemote(merged);
    ...
    if (!validStateReply(body) || body.ok !== true || body.revision !== sentRevision + 1 || !same(body.state, state)) throw unconfirmedStateReply();
  ...
  async function pullLatest({ manual = false } = {}) {
    ...
    try {
      ...
      if (protective) {
        ...
        acceptRemoteState(remoteState, {...meta, baseState: remoteState, revision: remoteRevision, lastSyncedAt: remote.updatedAt || isoNow()});
        showProtection();
        return;
      }
      ...
      acceptRemoteState(merged, {...meta, baseState: normalizeState(remote.state), revision: remoteRevision, lastSyncedAt: remote.updatedAt || isoNow()});
  ...
  async function initialSync() {
    ...
    try {
      ...
      if (remote.state) {
        ...
        acceptRemoteState(chosen, {...meta, baseState: clone(remoteState), revision: remote.revision, lastSyncedAt: remote.updatedAt || isoNow()});
        ready = true;
        if (needsUpload) queued = true;
      } else {
        persistMeta({...meta, revision: 0, baseState: null});
        ready = true;
