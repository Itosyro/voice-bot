(() => {
  ...
  let state = loadState();
  const jumpProfileFields = {"jumpAge":"age","jumpHeight":"heightCm","jumpCurrentWeight":"currentWeightKg","jumpWeightTarget":"weightTargetKg","jumpCurrentApproach":"currentApproachJumpCm","jumpTargetApproach":"targetApproachJumpCm","jumpStandingReach":"standingReachCm","jumpSquatWeight":"squatWeightKg","jumpSquatReps":"squatReps","jumpScoliosisGrade":"scoliosisGrade","jumpCobbAngle":"cobbAngleDeg","jumpPhase":"phase","jumpBlockWeek":"programWeek","jumpMedicalStatus":"medicalStatus","jumpPrimaryRole":"primaryRole","jumpSecondaryRole":"secondaryRole","jumpCurvePattern":"curvePattern","jumpSymptoms":"symptoms","jumpMedicalNotes":"medicalNotes","jumpVolleyballCoach":"volleyballCoach","jumpStrengthCoach":"strengthCoach","jumpCoachNotes":"coachNotes","jumpReviewer":"reviewedBy","jumpReviewerRole":"reviewedRole","jumpReviewNotes":"coachNotes"};
  ...
  function updateStateObject(target, next) {
    ...
    for (const [key, value] of Object.entries(next)) {
      const old = Object.hasOwn(target, key) ? target[key] : undefined;
  ...
  function navigate(view) {
    ...
    $('#viewTitle').textContent = VIEW_COPY[state.tone === 'calm' ? 'calm' : 'direct'][view];
    window.scrollTo({ top: 0, behavior: 'smooth' });
    if (view === 'home') { renderHealthToday(); renderHeader(); renderCheckin(); renderSuggestion(); renderDailyPlan(); }
  ...
  function renderHeader() {
    ...
    $('#viewTitle').textContent = VIEW_COPY[state.tone === 'calm' ? 'calm' : 'direct'][activeView];
    const checkin = getTodayCheckin();
    ...
    container.innerHTML = `
      <div class="next-step-body">
        ...
        <div class="next-step-actions">
          <button class="button button-primary big" data-action="start-suggested" data-task-id="${task.id}" data-duration="${duration}">${VIEW_COPY[state.tone === 'calm' ? 'calm' : 'direct'].suggestion}</button>
  ...
  function renderFocus() {
    ...
    $('#timerMainButton').textContent = focus.running ? 'Пауза' : focus.remainingSeconds < focus.totalSeconds ? 'Продолжить' : VIEW_COPY[state.tone === 'calm' ? 'calm' : 'direct'].start;
  ...
  function socialFillForm(row=null) {
    rememberForm('socialContentForm');
  ...
  function aiApprovalActionLabel(action) {
    return ({
      ...
      health_sleep: 'Сон', health_checkin: 'Самочувствие', health_supplement_schedule: 'Расписание добавок', health_supplement_mark: 'Отметка добавки',
      day_plan: 'План дня'
    })[action] || 'Изменение';
  }
  ...
  function aiApprovalSend(proposalId, decision) {
    ...
    if (state.aiProposals?.some(p => String(p.id) === String(proposalId) && p.action?.startsWith("health_"))) command.healthVersion = 1;
  ...
  const renderJumpWithState = renderJumpLab;
