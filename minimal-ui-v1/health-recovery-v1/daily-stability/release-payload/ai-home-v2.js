(() => {
  ...
  async function request(token, method = 'GET', revision, state) {
    ...
    try {
      ...
      if (method === 'PUT') {
        // A proxy's {ok:true} or an unrelated snapshot is not a save receipt.
  ...
  function saveDraft() {
    try {
      ...
      // Exclude only a dispatched ambiguous write. A failed preflight GET has
  ...
  function manualTarget() { return '/manual.html?v=20260914-daily-stability-1'; }
