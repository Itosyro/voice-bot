...
- [x] Цветные кнопки через `style=` параметр (Bot API 9.4, aiogram 3.25+):
  ...
  - Режимы, действия, языки → без цвета (дефолтный)
...
Все найденные проблемы (30 штук за 5 сессий) исправлены. UI обновлён Unicode-символами и цветными кнопками. Код чист, линтер проходит.
