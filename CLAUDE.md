...
- **Доставка ответа** — прогресс-сообщение («✨ Полирую…») затем редактируется в финальный результат через `send_result` из `src/handlers/_reply.py`. Стриминг-превью (`sendMessageDraft`) убран: эфемерный драфт надо финализировать через `SendMessage`, а мы редактируем другое сообщение — из-за этого в чате оставались «осиротевшие» пустые превью-пузыри. `on_delta` в сервисах сохранён (default `None`) для будущего корректного стриминга.
- **Парс-моды** — у бота default `parse_mode=None` (вывод LLM может содержать `<`, `&`, `*` — нельзя парсить как HTML/Markdown). Готовые текстовые константы с HTML-тегами отправляются с явным `parse_mode="HTML"`; результат и транскрипт — без парс-мода (plain).
