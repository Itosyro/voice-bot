...
**Выход:** результат — моноширинный тап-чтобы-скопировать блок (`<code>`, без
рамки цитаты `<blockquote>`).
...
- **`parse_mode=None` у бота по умолчанию** (`create_bot`). Сырой вывод LLM может
  ...
  - **результат** экранируется `escape_html` и кладётся в `<code>…</code>` с явным
    `parse_mode="HTML"` → тап-чтобы-скопировать без рамки цитаты (раньше был
    `<blockquote><code>`, убрали по просьбе владельца — рамка не нужна);
...
- **Groq retry**: до 3 попыток, паузы 2с/4с; при 429 (`is_rate_limit_error`) **и**
  при 401 invalid_api_key (`is_auth_error`) — ротация по всем ключам из
  `get_all_groq_keys()` — и в LLM (`complete`), и в STT (`transcribe`). Так один
  невалидный/отозванный ключ под конкретный режим не валит весь режим целиком,
  если в `get_all_groq_keys()` есть другие рабочие ключи.
- **Whisper `prompt=`-хинт** (`transcribe.py:_TRANSCRIBE_PROMPT_HINT`): просим
  Whisper писать английские технические термины (prompt, API, backend…), вставленные
  в русскую речь, обычными словами в строке, а не отдельным капсом/заголовком —
  плюс полностью английскую речь транскрибировать на английском. Промпты
  `polish`/`prompt_eng` дополнены тем же правилом (язык ответа = язык входа,
  термины — как есть).
...
14. **Groq STT 401 Invalid API Key валил ВСЕ голосовые режимы.** В проде
    `transcribe()` падал с `401 invalid_api_key` 3 раза подряд (ротация ключей была
    только на 429) → `voice_handler_error`, пользователь видел `GROQ_ERROR`. Один
    битый/отозванный ключ под режим (`get_groq_key(mode)`) делал нерабочим весь этот
    режим целиком. → Добавили `is_auth_error` в `llm.py`, ротация ключей теперь и на
    401, и на 429 (в `transcribe` и `complete`). Если ВСЕ ключи в проде невалидны —
    нужно проверить/обновить `GROQ_API_KEY_*` в Render env, кодом это не лечится.
...
- **#13** (`df975c7`) — переписан CLAUDE.md в подробный мастер-план.
- **#14** — фикс 401 invalid_api_key (валил все голосовые режимы): ротация Groq-ключей
  и на 401, и на 429 (`is_auth_error` в `llm.py`); убрана рамка `<blockquote>` у
  результата (остался `<code>` для тап-копирования); Whisper `prompt=`-хинт +
  правила в `polish`/`prompt_eng` для англ. терминов посреди русской речи и для
  языка ответа = языку входа.
